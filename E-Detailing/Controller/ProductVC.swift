...
class ProductVC : UIViewController {
    ...
    //    self.updateDisplay()
        ...
        if appsetup.sampleValidation == 1 || appsetup.inputValidation == 1 {
            self.totalStock()
        }
    }
    ...
    func totalStock() {
        ...
        let stocks = DBManager.shared.getStockBalance()
        
        guard let inputStockValue = stocks, let inputLists = inputStockValue.input?.allObjects as? [InputStockBalance] else {
            return
        }
        guard let productStockValue = stocks , let productLists = productStockValue.product?.allObjects as? [ProductStockBalance] else {
            return
        }
        
        for product in productLists {
            
            let products = DBManager.shared.getProduct().filter{$0.code == product.code}
            
            if !products.isEmpty {
                let balanceStock = product.balanceStock <= 0 ? 0 : product.balanceStock
                productStocks.append(Stocks(object: products.first!, totalCount: "\(balanceStock)", availableCount: "\(balanceStock)", mainProductCount: "0"))
            }
        }
        
        for input in inputLists {
            
            let inputs = DBManager.shared.getInput().filter{$0.code == input.code}
            
            if !inputs.isEmpty {
                let balanceStock = input.balanceStock <= 0 ? 0 : input.balanceStock
                inputStocks.append(Stocks(object: inputs.first!, totalCount: "\(balanceStock)", availableCount: "\(balanceStock)", mainProductCount: "0"))
            }
        }
...
//        self.viewPobAndFeedbackCollection.widthAnchor.constraint(equalTo: self.viewFeedback.widthAnchor, multiplier: 0.24).isActive = true
    ...
    @IBAction func submitAction(_ sender: UIButton) {
        ...
        let timesLine = ["sTm" : date , "eTm" : date]
...
//let times = ["eTm" : "", "sTm" : ""]
    ...
    //    let product : [String : Any] = ["Code" : "" , "Name" : "", "Group" : "", "ProdFeedbk" : "", "Rating" : "", "Timesline" : "timesLine", "Appver" : "", "Mod" : "", "SmpQty" : "", "RxQty" : "" , "prdfeed" : "", "Type" : "", "StockistName" : "", "StockistCode" : "", "Slides" : "slides"]
        ...
        for product in productValue {
            let product : [String : Any] = ["Code" : product.code , "Name" : product.name, "Group" : "", "ProdFeedbk" : "", "Rating" : "", "Timesline" : timesLine, "Appver" : "iEdet.1.1", "Mod" : "iOS-Edet-New", "SmpQty" : product.sampleCount, "RxQty" : product.rxCount ,  "Type" : cusType, "StockistName" : "", "StockistCode" : "", "Slides" : []]
...
//            self.rcpaAddedListViewModel.addRcpaCompetitorProductAtProduct(0, row: 0, product: RcpaHeaderData(chemist: self.selectedChemistRcpa, product: self.selectedProductRcpa, quantity: self.txtRcpaQty.text ?? "", total: self.txtRcpaTotal.text ?? "", rate: self.txtRcpaRate.text ?? "", competitorCompanyName: "DOLO", competitorCompanyCode: "", competitorBrandName: "", competitorBrandCode: "DOLO", competitorRate: "5", competitorTotal: "50", competitorQty: "10",remarks: ""))
    ...
    @IBAction func addAdditionalCallSampleInput(_ sender: UIButton) {
        ...
        case 0:
            ...
            break
        case 1:
            self.additionalCallListViewModel.addInputAtIndex(self.selectedDoctorIndex, vm: InputViewModel(input: InputData(input: nil, availableCount: "", totalCount: "", inputCount: "")))
            self.additionalCallSampleInputTableView.reloadData()
            break
        default:
            break
...
extension ProductVC : tableViewProtocols {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case self.productSampleTableView :
            ...
            let product = self.productSelectedListViewModel.fetchDataAtIndex(indexPath.row)
            cell.productSample = product
            ...
            if let index = self.productStocks.firstIndex(where: { stocks in
                stocks.object.code == product.code
            }){
                
                let totalCount = Int(self.productStocks[index].totalCount) ?? 0
                
                let additionalCall = self.productStocks[index].additionalDoctorStocks.map{$0.sampleQty}.reduce(0){$0 + $1}
                
                let mainProductCount = Int(self.productStocks[index].mainProductCount) ?? 0
                
                let availableCount = totalCount - (additionalCall + mainProductCount)
                
                if  availableCount >= 0{
                    cell.txtProductCount.text = "\(availableCount)"
                }
            }
            return cell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case self.ProductTableView :
            ...
            if productValue.isSelected {
                ...
                let product = self.productAvailableQty.filter{$0.code == selectedProdcutCode}
        ...
        case self.inputListTableView:
            ...
            if inputValue.isSelected {
                ...
                if appsetup.inputValidation == 1 {
                    let selectedInput = inputValue.Object as? Input
                    
                    let selectedInputCode = selectedInput?.code
                    
                    let input = self.inputAvailableQty.filter{$0.code == selectedInputCode}
                    ...
                    if !input.isEmpty {
                        ...
                        let cnt = input.first!.balanceStock <= 0 ? 0 : input.first!.balanceStock
                        count = "\(cnt)"
                        availableCount = "\(cnt - 1)"
                    }
                    
                    self.inputSelectedListViewModel.addInputViewModel(InputViewModel(input: InputData(input: inputValue.Object as? Input, availableCount: availableCount, totalCount: count, inputCount: "1")))
                    ...
                    self.inputSelectedListViewModel.addInputViewModel(InputViewModel(input: InputData(input: inputValue.Object as? Input, availableCount: "", totalCount: "", inputCount: "1")))
        ...
        switch self.SampleInputSegmentControl.selectedSegmentIndex{
            case 0:
                ...
                selectionVC.didSelectCompletion { selectedIndex in
                    ...
                    let selectedProductCode = selectedProduct.code
                    
                    let product = self.productAvailableQty.filter{$0.code == selectedProductCode}
                    
                    var count = ""
                    
                    if !product.isEmpty {
                        let cnt = product.first!.balanceStock <= 0 ? 0 : product.first!.balanceStock
                        count = "\(cnt)"
                    }
                    
                    
                    if let index = self.productStocks.firstIndex(where: { stocks in
                        stocks.object.code == selectedProduct.code
                    }){
                        let totalCount = Int(self.productStocks[index].totalCount) ?? 0
                        ...
                        print("count \(additionalCall)")
                        
                        let availableCount = totalCount - (mainProductCount + additionalCall)
                        ...
                        if let cell = self.additionalCallSampleInputTableView.cellForRow(at: indexPath) as? AdditionalCallSampleEntryTableViewCell {
                            ...
                            cell.txtAvailableStock.text = "\(availableCount)"
                        }
                        ...
                        self.additionalCallListViewModel.updateProductAvailableCountAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: "\(availableCount)")
                        self.additionalCallListViewModel.updateProductTotalCountAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: "\(availableCount)")
                    }
                    
//                    if let cell = self.additionalCallSampleInputTableView.cellForRow(at: indexPath) as? AdditionalCallSampleEntryTableViewCell {
//                        cell.lblName.text = products[selectedIndex].name
//                        cell.txtAvailableStock.text = count
//                    }
//                    
//                    self.additionalCallListViewModel.updateProductAtSection(self.selectedDoctorIndex, index: indexPath.row, product: products[selectedIndex])
//                    
//                    self.additionalCallListViewModel.updateProductAvailableCountAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: count)
//                    self.additionalCallListViewModel.updateProductTotalCountAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: count)
            ...
            case 1:
                ...
                selectionVC.didSelectCompletion { selectedIndex in
                    ...
                    let selectedInput = inputs[selectedIndex]
                    ...
                    self.additionalCallListViewModel.updateInputAtSectionAvailableCount(self.selectedDoctorIndex, index: indexPath.row, count: count)
                    
                    self.additionalCallListViewModel.updateInputAtSectionTotalCount(self.selectedDoctorIndex, index: indexPath.row, count: count)
    ...
    @objc func updateSampleInputQty(_ sender : UITextField) {
        ...
        switch self.SampleInputSegmentControl.selectedSegmentIndex {
            case 0:
                ...
                let product = self.additionalCallListViewModel.fetchProductAtIndex(self.selectedDoctorIndex, index: indexPath.row)
            
            let doctor = self.additionalCallListViewModel.fetchDataAtIndex(self.selectedDoctorIndex)
            
                print(product)
                print(doctor.docName)
                print(doctor.docCode)
...
//                    let totalCount = Int(self.productStocks[index].totalCount) ?? 0
//                
//                    let sample = Int(sender.text ?? "0") ?? 0
//                    
//                    let mainProductCount = Int(self.productStocks[index].mainProductCount) ?? 0
//                    
//                    let additionalCall = self.productStocks[index].additionalDoctorStocks.map{$0.sampleQty}.reduce(0){$0 + $1}
...
//                    let availableCount = totalCount  - (mainProductCount + additionalCall)
//                
//                    if sample > totalCount || availableCount == 0 {
//                        if let cell = self.additionalCallSampleInputTableView.cellForRow(at: indexPath) as? AdditionalCallSampleEntryTableViewCell {
//                            cell.txtAvailableStock.text = "\(availableCount)"
//                            cell.txtSampleStock.text = ""
//                        }
//                        return
//                    }
//                        
//                    let value = availableCount - sample
//                
//                    self.additionalCallListViewModel.updateProductAvailableCountAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: "\(value)")
//                    self.additionalCallListViewModel.updateProductQtyAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: sender.text ?? "")
//                
//                    if let cell = self.additionalCallSampleInputTableView.cellForRow(at: indexPath) as? AdditionalCallSampleEntryTableViewCell {
//                        cell.txtAvailableStock.text = "\(value)"
//                        cell.txtSampleStock.text = sender.text ?? ""
//                    }
//                    
//                    if let firstindex =  self.productStocks[index].additionalDoctorStocks.firstIndex(where: { additionalCallStocks in
//                        additionalCallStocks.sampleProduct == product.code && additionalCallStocks.doctor == doctor.docCode
//                    }){
//                        self.productStocks[index].additionalDoctorStocks[firstindex].sampleQty = sample
//                    }else{
//                        self.productStocks[index].additionalDoctorStocks.append(additionalCallStocks(doctor: doctor.docCode, doctorIndex: self.selectedDoctorIndex, sampleProduct: product.code, sampleQty: sample))
//                    }
//                    
//                }
            
//                let totalCount = Int(product.totalCount) ?? 0
//            
//                let sample = Int(sender.text ?? "0") ?? 0
//            
//                if sample > totalCount || totalCount == 0 {
//                    if let cell = self.additionalCallSampleInputTableView.cellForRow(at: indexPath) as? AdditionalCallSampleEntryTableViewCell {
//                        cell.txtAvailableStock.text = "\(totalCount)"
//                        cell.txtSampleStock.text = ""
//                    }
//                    return
//                }
//                    
//                let value = totalCount - sample
//            
//                self.additionalCallListViewModel.updateProductAvailableCountAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: "\(value)")
//                self.additionalCallListViewModel.updateProductQtyAtSection(self.selectedDoctorIndex, index: indexPath.row, qty: sender.text ?? "")
//            
...
//                let input = self.additionalCallListViewModel.fetchInputAtIndex(self.selectedDoctorIndex, index: indexPath.row)
//            
//                let totalCount = Int(input.totalCount) ?? 0
//                
//                let sample = Int(sender.text ?? "0") ?? 0
//            
//                if sample > totalCount || totalCount == 0 {
...
//                let value = totalCount - sample
//            
//                self.additionalCallListViewModel.updateInputAtSectionAvailableCount(self.selectedDoctorIndex, index: indexPath.row, count: "\(value)")
...
//            print("count \(additionalCall)")
//            
//            let availableCount = totalCount - additionalCall
//            
//            if sample > totalCount || availableCount == 0{
//                if let cell = self.productSampleTableView.cellForRow(at: indexPath) as? ProductSampleTableViewCell{
//                    cell.txtProductCount.text = "\(availableCount)"
//                    cell.txtSampleQty.text = ""
//                }
//                return
//            }
//            
//            let value = availableCount - sample
//            
//            self.productSelectedListViewModel.setAvailableCountAtIndex(sender.tag, qty: "\(value)")
//            
//            self.productSelectedListViewModel.setSampleCountAtIndex(sender.tag, qty: sender.text ?? "")
//            
//            if let cell = self.productSampleTableView.cellForRow(at: indexPath) as? ProductSampleTableViewCell {
//                cell.txtProductCount.text = "\(value)"
//                cell.txtSampleQty.text = sender.text ?? ""
//            }
//            
//            self.productStocks[index].mainProductCount = sender.text ?? ""
//        }
        ...
        if appsetup.sampleValidation == 1{
            let totalCount = Int(productStocks.first!.totalCount) ?? 0
            
            let sample = Int(sender.text ?? "0") ?? 0
            
            if sample > totalCount || totalCount == 0 {
                if let cell = self.productSampleTableView.cellForRow(at: indexPath) as? ProductSampleTableViewCell{
                    cell.txtProductCount.text = "\(totalCount)"
                    cell.txtSampleQty.text = ""
                }
                return
            }
    ...
    @objc func updateInputSampleQty(_ sender : UITextField){
        ...
        let input = self.inputSelectedListViewModel.fetchDataAtIndex(indexPath.row)
        ...
        if appsetup.inputValidation == 1 {
            ...
            let inputQty = Int(sender.text ?? "0") ?? 0
            
            if inputQty > totalCount || totalCount == 0 {
                if let cell = self.inputSampleTableView.cellForRow(at: indexPath) as? InputSampleTableViewCell {
                    cell.txtInputCount.text = "\(totalCount)"
                    cell.txtSampleQty.text = ""
                }
                return
            }
            
            let value = totalCount - inputQty
            
            self.inputSelectedListViewModel.setAvailableCountAtIndex(indexPath.row, count: "\(value)")
            
            self.inputSelectedListViewModel.setInputCodeAtIndex(indexPath.row, samQty: sender.text ?? "")
            
            if let cell = self.inputSampleTableView.cellForRow(at: indexPath) as? InputSampleTableViewCell {
                
                cell.txtInputCount.text = "\(value)"
                cell.txtSampleQty.text = sender.text ?? ""
            }
        }else {
            self.inputSelectedListViewModel.setInputCodeAtIndex(indexPath.row, samQty: sender.text ?? "")
...
struct Stocks {
    ...
    var mainProductCount : String
    var additionalDoctorStocks  = [additionalCallStocks]()
}
